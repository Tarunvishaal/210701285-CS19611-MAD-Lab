...
class MainActivity : AppCompatActivity() {
    ...
    private fun deleteData() {
        ...
        val deletedRows = db?.delete(DBContract.StudentEntry.TABLE_NAME, selection, selectionArgs)

        Toast.makeText(this, "Deleted $deletedRows rows", Toast.LENGTH_SHORT).show()
    }
    ...
    private fun viewData() {
        ...
        while (cursor?.moveToNext() == true) {
            val registerNumber = cursor.getInt(cursor.getColumnIndexOrThrow(DBContract.StudentEntry.COLUMN_REGISTER_NUMBER))
            val name = cursor.getString(cursor.getColumnIndexOrThrow(DBContract.StudentEntry.COLUMN_NAME))
            val cgpa = cursor.getDouble(cursor.getColumnIndexOrThrow(DBContract.StudentEntry.COLUMN_CGPA))
